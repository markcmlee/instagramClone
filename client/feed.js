...
$(document).ready(() => {
  function updateHeart(heartId) {
    ...
    heart.src = heart.src.includes("heartButton.png")
      ? "client/Images/likedHeart.png"
      : "client/Images/heartButton.png";
  }
    ...
    success: function (data) {
      data.forEach((imageSrc, i) => {
        ...
        if (!imageSrc.includes("fake")) {
          ...
          $(".feedbox").append(
            `<div class="feedposts" id='${postPicId}'>
                <img src="client/Images/Broken-images.png"  class="post" />
            </div>`
          );
        }
        ...
        $(`#${postPicId}`).append(
          `<div class="likebar">
              <img src="client/Images/heartButton.png" id="${heartId}" />
              <div id="${peopleWhoLikedPostId}">
              </div>
              <img src="client/Images/moreOptions.png" id="${moreOptionsId}" />
            </div>`
        );
        ...
        $(`#${formId}`).bind("submit", function (e) {
          ...
          $(`.${commentListClass}`).append(
            `<li style="font-family:sans-serif"><span id="usernameComment">marklee</span> ${commentField.value}</li>`
          );
        ...
        const randomizeAndGenerateLikes = (array) => {
          ...
          $(`#${peopleWhoLikedPostId}`).append(
            `<p>
            <span id="usernameComment">${array[0]}, ${array[1]}, ${
              array[2]
            }</span>
            and <span style="color:gray; font-weight:700">${Math.floor(
              Math.random() * 500
            )}</span> others liked this post.
            </p>`
          );
        };
        randomizeAndGenerateLikes(usernameList);

        const randomizeAndGeneratePostComments = (
          array1,
          array2,
          array3,
          array4
        ...
        randomizeAndGeneratePostComments(
          usernameList,
          commentList,
          hashtagList,
          userImages
